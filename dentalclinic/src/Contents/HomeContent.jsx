...
let homedata = [
    {
        ...
        para : "World-class dental care delivered with compassion. complete smile makeovers — we've got you covered ",
        btn1 : "Our Services",
...
{ homedata.map((items , index)=>{
 ...
 <div className="flex flex-col gap-1 px-3">
  <Heading text={items.headingone} className="w-full text-4xl lg:text-5xl leading-tight text-[var(--secondary)]"/>
  <Heading text={items.headingtwo} className="w-full text-4xl lg:text-5xl leading-tight text-[var(--secondary)]"/>
</div>
    {/* <Heading text={items.headingtwo} className="text-4xl lg:text-5xl  px-3 py-1 text-[var(--secondary)]"/> */}
    </div>
   <div className='px-4  pt-4 lg:px-11 lg:pt-3 w-full'>
<p className='text-[var(--text)] text-xl'>
    {items.para}
</p>
   </div>
   <div className='flex gap-10 lg:mx-10 mx-4 pt-5 lg:pt-4'>
    <Button className="bg-gradient-to-r from-[#278981]  to-[rgb(28,150,140)]" text={items.btn1}/>
    <Button className="border-1 text-[var(--text)] border-gray-400 px-4 py-2 " text={items.btn2}/>
   </div>
            </div>
        )
    })
}
<div className='w-full lg:mx-10 pt-10 lg:pt-5 flex lg:gap-5 gap-3 '>
